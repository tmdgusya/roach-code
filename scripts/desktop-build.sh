...
#   macOS:   Roach Code-darwin-<arch>.zip                  (ditto archive of the .app)
#   Windows: Roach Code-windows-<arch>-installer.exe       (NSIS per-user installer)
#   Linux:   Roach Code-linux-<arch>.tar.gz                (bare binary)
#
...
APPNAME="Roach Code"            # wails.json productName -> Roach Code.app
BINNAME="roach-code-desktop"    # wails.json outputfilename -> linux binary name
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (roach-code-desktop.app); repackage
	# it as Roach Code.app for a clean user-facing name. Ad-hoc sign the copy (still
	...
	cp -R "build/bin/roach-code-desktop.app" "$app"
