...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "roach-code.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Roach Code e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "roach-code/internal/agent/foo.go" into
