...
import (
	...
	"roach-code/internal/codegraph"
	"roach-code/internal/config"
)

// mcp.go holds the MCP server-management surface shared by the `roach-code mcp`
...
// mcpCommand implements `roach-code mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if cfg.Codegraph.Enabled {
		if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
			...
			fmt.Printf("%-16s (built-in, not installed)  run `roach-code codegraph install` (or it auto-installs on first use)\n", "codegraph")
		}
		listed++
	}
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: roach-code mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to roach-code.toml).

Usage:
  roach-code mcp list
  roach-code mcp add <name> <command> [args...]        stdio server
  roach-code mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  roach-code mcp add <name> --sse  <url>               remote (legacy SSE)
  roach-code mcp remove <name>
...
Examples:
  roach-code mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  roach-code mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
