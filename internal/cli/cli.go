// Package cli implements roach-code's command-line entry: subcommand routing, flag
...
import (
	...
	"roach-code/internal/provider/openai"
	"roach-code/internal/serve"
	"time"
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `roach-code init` isn't a dead end.
	...
	case "version", "--version", "-v":
		fmt.Println("roach-code", version)
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runServe(args []string) int {
	...
	fmt.Printf("roach-code serve — %s on http://%s\n", ctrl.Label(), *addr)
...
func chatREPL(args []string, version string) int {
	...
	if cfg, err := config.Load(); err == nil {
		...
		m.bashSandbox = bashSandboxFromConfig(cfg) // bash confinement summary shown on a bash gate
	}
...
// initHint handles `roach-code init`. Unlike a config scaffold, project memory is
...
// (and to `roach-code setup` for config) so the verb isn't a dead end.
func initHint() int {
...
func interactiveSetup(path string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("roach-code")),
		"",
...
// that older versions of the wizard wrote into roach-code.toml. They collide
...
// can warn the user to clean up roach-code.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `roach-code setup`.
// The host-based slug also gives users a meaningful name to grep for in
// roach-code.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing roach-code.toml deal with SKU names
...
func familyOf(name string) providerFamily {
	...
	case strings.HasPrefix(name, "minimax"):
		return providerFamily{key: "minimax", name: "MiniMax", desc: "M2 — agentic & coding, cheap"}
	case strings.HasPrefix(name, "glm"):
		return providerFamily{key: "glm", name: "GLM (Z.ai)", desc: "strong coding, thinking mode"}
	case strings.HasPrefix(name, "codex") || strings.HasPrefix(name, "gpt"):
		return providerFamily{key: "codex", name: "OpenAI Codex", desc: "GPT/Codex via Responses API"}
	default:
...
// roach-code.toml that defines only [[providers]] for deepseek otherwise hides
...
func configureKeys(selected []config.ProviderEntry, r io.Reader, w io.Writer) []string {
	...
	for _, p := range selected {
		// OAuth-mode providers (codex via ChatGPT login) don't use an env key —
		// don't prompt for one; point the user at the login command instead.
		if p.AuthMode() == config.AuthOAuth {
			if !loginHinted {
				fmt.Fprintf(w, "  %s %s\n", green("✓"), "codex uses ChatGPT login — run `roach-code codex login` (no API key needed)")
				loginHinted = true
			}
			continue
		}
...
func welcome(version string) int {
	...
	// roach-code.toml exists and parses on a terminal: go into chat. If any enabled
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("roach-code") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("roach-code setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("roach-code chat", i18n.M.StepChatDesc)
	step(`roach-code run "task"`, i18n.M.StepRunDesc)
