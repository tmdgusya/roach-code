...
// codegraphCommand backs `roach-code codegraph` — managing the CodeGraph
// code-intelligence runtime that roach-code otherwise fetches lazily on first use.
...
func codegraphStatus() int {
	...
	if p, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-13s %s\n", "resolved:", "(not installed — run `roach-code codegraph install`)")
	}
	return 0
}

func codegraphUsage() {
	fmt.Print(`roach-code codegraph — manage the CodeGraph code-intelligence runtime

Usage:
  roach-code codegraph install   download + cache the runtime for this platform
  roach-code codegraph status    show config, cache dir, and resolved launcher
