...
// provider/model refs (built-in defaults when no roach-code.toml is present), and
...
func TestModelRefsSkipsUnconfigured(t *testing.T) {
	...
	// Isolate the user-config dir so a real codex-auth.json on the dev machine
	// doesn't make the OAuth-capable codex preset count as configured.
	cfgDir := t.TempDir()
