...
import (
	...
	"roach-code/internal/billing"
	"roach-code/internal/checkpoint"
...
// SummarizeUpTo compresses everything before it. Both are the "summarize
...
// SessionCost returns the session's cumulative spend estimate and the currency
// symbol to render it with (0, "" when the active model has no pricing
// configured), so a frontend can show a live running-total cost on the status
// line.
func (c *Controller) SessionCost() (cost float64, symbol string) {
	if c.executor == nil {
		return 0, ""
	}
...
// ROACH-CODE.md by default) — the write side of "#<note>". Returns the file written.
