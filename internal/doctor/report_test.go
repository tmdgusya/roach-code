...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("ROACH_TEST_SECRET", "sk-live-secret")
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	...
	if !strings.Contains(text, "roach-code 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config roach-code.toml: parse boom"}})
