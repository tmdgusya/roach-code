...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w %q (configured: %s); note: defining [[providers]] replaces the built-in presets, so add a [[providers]] entry for it or use a configured name, or run `roach-code setup` to reconfigure", ErrUnknownModel, modelName, providerNames(cfg))
	}
	if opts.RequireKey {
	...
	if !opts.RequireKey && entry.APIKeyEnv != "" && entry.APIKey() == "" && !entry.OAuthCapable() {
	...
	if cfg.Codegraph.Enabled {
		...
		default:
			sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelInfo,
				Text: "codegraph: not installed — run `roach-code codegraph install` to enable symbol-graph tools"})
		}
	}
	...
	// resolves "ask" to allow — preserving `roach-code run` autonomy — while deny
	...
	// Custom slash commands (.roach-code/commands + user dir). Best-effort: a malformed
