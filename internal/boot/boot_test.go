...
import (
	...
	// cmd/roach-code's main does; without them Build sees an empty provider
...
// cache-first wiring: a project ROACH-CODE.md is discovered at boot and folded
...
func TestBuildFoldsProjectMemoryIntoSystemPrompt(t *testing.T) {
	...
	writeFile(t, dir, "roach-code.toml", `
...
model = "x"
api_key_env = "ROACH_TEST_KEY_UNSET"
`)
	writeFile(t, dir, "ROACH-CODE.md", "Project rule: always run go vet before committing.")
	...
	if !strings.Contains(sys, "always run go vet before committing") {
		t.Fatalf("project ROACH-CODE.md not folded into system message:\n%s", sys)
	}
	...
	if mem := ctrl.Memory(); mem == nil || len(mem.Docs) == 0 {
		t.Fatal("controller memory set is empty after discovering ROACH-CODE.md")
	}
}
...
api_key_env = "ROACH_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".roach-code/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
[[plugins]]
name = "missing"
command = "roach-code-missing-mcp-binary"
tier = "eager"
`)
...
func TestBuildEagerStartsAtBoot(t *testing.T) {
	...
	writeFile(t, dir, "roach-code.toml", fmt.Sprintf(`
default_model = "test-model"
...
[[plugins]]
name = "slowserver"
command = "roach-code-missing-slow-mcp-binary"
tier = "eager"
`)
