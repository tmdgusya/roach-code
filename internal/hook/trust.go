...
// Trust gates project hooks. A project's .roach-code/settings.json can run
...
// root. The trust flag lives in user-global state (~/.roach-code/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.roach-code/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.roach-code.
...
// TrustPath is ~/.roach-code/trust.json (homeDir overrides ~).
