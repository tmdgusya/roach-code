...
// (.roach-code/settings.json, only when the project is trusted) and a global
// (~/.roach-code/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".roach-code"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.roach-code/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.roach-code/settings.json.
