...
// <user cache>/roach-code/codegraph/<Version>. Versioned so a bump installs cleanly
// beside the old one. ROACH_CACHE_DIR overrides the base (relocate the cache,
// or isolate it in tests). Empty when no cache/config dir resolves.
func CacheDir() string {
	base := os.Getenv("ROACH_CACHE_DIR")
	if base == "" {
		var err error
		...
		base = filepath.Join(base, "roach-code")
	}
...
// InstallWithClient is Install with an explicit HTTP client, used when Roach Code
