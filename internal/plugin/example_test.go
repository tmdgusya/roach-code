...
// buildExamplePlugin compiles cmd/roach-code-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "roach-code-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "roach-code/cmd/roach-code-plugin-example").CombinedOutput()
	if err != nil {
