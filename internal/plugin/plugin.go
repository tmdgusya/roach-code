// Package plugin is Roach Code's MCP client. It connects to external MCP servers and
...
// protocolVersion is the MCP revision Roach Code advertises during initialize.
...
type Spec struct {
	...
	// inherit roach-code's cwd (the default for user-configured plugins). It exists
...
// messages (notifications, requests like roots/list) are ignored — Roach Code is a
...
func (c *Client) initialize(ctx context.Context) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "roach-code", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into Roach Code's parallel-dispatch
...
// matching the MCP convention. Spaces in either part are normalised to underscores so
