...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "roach-code-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "roach-code-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "roach-code-missing-mcp-binary" not found on PATH`,
