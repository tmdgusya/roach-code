...
func New(cfg provider.Config) (provider.Provider, error) {
	...
	// Emit the `thinking:{type:enabled}` request field for DeepSeek (always on) and
	// for any other OpenAI-compatible host that opts in via thinking="enabled" —
	// e.g. GLM/Z.ai, whose reasoning toggle uses the same shape. Anything else
	// (the empty default, or "off") leaves the field off so non-thinking gateways
	// aren't sent a parameter they'd reject.
	emitThinking := deepseek || strings.EqualFold(strings.TrimSpace(thinking), "enabled")
...
type client struct {
	...
	http     *http.Client
	deepseek bool
	thinking bool   // emit thinking:{type:enabled} (DeepSeek always; others via thinking="enabled")
...
func (c *client) buildRequest(req provider.Request) chatRequest {
	...
	// Emit thinking when explicitly enabled (GLM/Z.ai et al.) or for DeepSeek,
	// whose reasoning is always on (the deepseek flag also guards direct client
	// construction that bypasses New).
	if c.thinking || c.deepseek {
