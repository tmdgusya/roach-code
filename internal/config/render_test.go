...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${ROACH_PROXY_PASSWORD}",
		},
	}
	orig.Skills.Paths = []string{"~/my-skills", "../shared/skills"}
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "roach-code-plugin-example"},
	...
	// auth must round-trip — dropping it silently reverts codex from OAuth to
	// API-key auto-detect, which is the bug this assertion guards against.
	if g, _ := got.Provider("codex"); g == nil || g.Auth != "oauth" {
		t.Errorf("codex auth not preserved: %+v", g)
	}
