...
// RenderTOML renders the config as annotated TOML in the `roach-code setup` house style:
...
func RenderTOML(c *Config) string {
	...
	b.WriteString("# Roach Code configuration.\n")
	b.WriteString("# Resolution order: flag > ./roach-code.toml > ~/.config/roach-code/config.toml > built-in defaults.\n")
	b.WriteString("# Secrets come from the environment via api_key_env; never put keys here.\n\n")

	fmt.Fprintf(&b, "default_model = %q\n", c.DefaultModel)
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $ROACH_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $ROACH_LANG\n")
	}
	b.WriteString("\n")

	b.WriteString("[ui]\n")
	fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; ROACH_THEME can override per run\n", c.UITheme())
	if style := c.UIThemeStyle(); style != "" {
		fmt.Fprintf(&b, "theme_style = %q   # accent palette; ROACH_THEME_STYLE can override per run\n", style)
	} else {
	...
	if c.Network.Proxy.Password != "" {
		...
		b.WriteString("# password = \"${ROACH_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
	}
	b.WriteString("\n")
	...
	for _, p := range c.Providers {
		...
		if p.Auth != "" {
			fmt.Fprintf(&b, "auth        = %q   # oauth (ChatGPT login) | apikey | \"\" auto\n", p.Auth)
		}
		b.WriteString("\n")
	}
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"roach-code-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `roach-code setup`
