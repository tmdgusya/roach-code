...
// mcpJSONFile is the project-root .mcp.json. Roach Code reads
...
// mcpServerSpec mirrors one entry of the "mcpServers" map. The field
// names and semantics match PluginEntry: command/args/env describe
...
// loadMCPJSON reads path (the project .mcp.json) and returns its servers as
...
// already declare. roach-code.toml's [[plugins]] win on a name collision: it is the
// Roach Code-specific, more explicit of the two, so it overrides the shared,
