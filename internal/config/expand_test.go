...
func TestExpandVars(t *testing.T) {
	t.Setenv("ROACH_TEST_TOKEN", "sk-123")
	t.Setenv("ROACH_TEST_EMPTY", "")
...
func TestExpandedPlugin(t *testing.T) {
	t.Setenv("ROACH_TEST_KEY", "secret")
	e := PluginEntry{
		Name:    "x",
		Type:    "http",
		URL:     "https://api/${ROACH_TEST_MISSING:-v1}",
		Args:    []string{"--token", "${ROACH_TEST_KEY}"},
		Env:     map[string]string{"K": "${ROACH_TEST_KEY}"},
		Headers: map[string]string{"Authorization": "Bearer ${ROACH_TEST_KEY}"},
	}
	...
	if e.Headers["Authorization"] != "Bearer ${ROACH_TEST_KEY}" {
