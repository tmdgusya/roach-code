...
// separate from the `roach-code setup` wizard (cli) so a GUI can apply one setting at a
...
// half-written roach-code.toml that fails to parse on next load. Parent directories
...
func (c *Config) SaveTo(path string) error {
	...
	tmp, err := os.CreateTemp(dir, ".roach-code.*.toml.tmp")
	if err != nil {
...
// (SourcePath), or to ./roach-code.toml when none exists yet — the conventional
