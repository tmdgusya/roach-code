...
// assistant content (including reasoning) are untouched by construction.
const (
...
// compaction is disabled (no context window). This is the free alternative to a
// paid summarize fold: it shrinks the prompt without an API call, which matters
// most on a cold-resume where the provider prefix cache has expired and the
// rewrite costs no extra cache misses.
