...
import (
	...
	"roach-code/internal/diff"
	"roach-code/internal/event"
	"roach-code/internal/evidence"
...
type Agent struct {
	...
	// sessUncachedIn/sessCompletion accumulate the billable token counts the
	// session-cost estimate is built from: uncached prompt input (charged at the
	// Input rate) and completion output. sessUncachedIn takes max(reportedMiss,
	// prompt−hit) per call so it stays correct even when a provider omits the miss
	// field on an all-uncached turn — normaliseUsage only derives miss when hit>0,
	// so the first turns would otherwise price their input at zero. Atomic for the
	// same reason as the cache counters above.
...
// `roach-code chat --resume` to load a saved JSONL transcript before the first turn,
...
// SessionCost estimates the session's cumulative spend across every API call,
// priced at the model's configured rates, and returns the currency symbol to
// render it with. Returns (0, "") when no pricing is configured — the status
// line then omits the readout. Built from the running token accumulators rather
// than a per-turn float sum so it survives compaction unchanged.
func (a *Agent) SessionCost() (cost float64, symbol string) {
	if a.pricing == nil {
		return 0, ""
	}
	u := provider.Usage{
		CacheHitTokens:   int(a.sessCacheHit.Load()),
		CacheMissTokens:  int(a.sessUncachedIn.Load()),
		CompletionTokens: int(a.sessCompletion.Load()),
	}
	return a.pricing.Cost(&u), a.pricing.Symbol()
}
...
func (a *Agent) stream(ctx context.Context, turn int) (string, string, string, []provider.ToolCall, *provider.Usage, error) {
	...
	for chunk := range ch {
		...
		case provider.ChunkUsage:
			...
			uncached := chunk.Usage.CacheMissTokens
			if in := chunk.Usage.PromptTokens - chunk.Usage.CacheHitTokens; in > uncached {
				uncached = in // all-uncached turn where the provider left miss unset
			}
			a.sessUncachedIn.Add(int64(uncached))
			a.sessCompletion.Add(int64(chunk.Usage.CompletionTokens))
