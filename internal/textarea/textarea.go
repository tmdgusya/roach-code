...
// applications.
//
// Vendored from charm.land/bubbles/v2@v2.1.0 (MIT, see LICENSE) so we can patch
// a soft-wrap bug: upstream wrap() drops the trailing rune of a no-space word
// when a double-width glyph straddles the wrap column (e.g. typing Korean and
// inserting mid-line made the last character vanish from the input box). The
// only behavioural change is inside wrap() — search for "roach-code fix". Keep
// the rest in sync with upstream; drop this vendor once the fix lands upstream.
package textarea
...
import (
	...
	"roach-code/internal/textarea/internal/memoization"
	"roach-code/internal/textarea/internal/runeutil"
)
	...
	for _, r := range runes {
		...
		if spaces > 0 { //nolint:nestif
			...
			if uniseg.StringWidth(string(word))+lastCharLen > width {
				...
				// roach-code fix: character-wrap the word instead of dumping it
				// whole. Upstream did `lines[row] = append(lines[row], word...)`,
				// which overflows the line when a double-width glyph straddles
				// the wrap column (cumulative width skips the boundary, e.g.
				// 11→13 cells at width 12) — the renderer/viewport then clips the
				// overflowing rune and it vanishes. Placing each rune only where
				// it fits pushes such a glyph to the next line instead.
				for _, r := range word {
					if uniseg.StringWidth(string(lines[row]))+rw.RuneWidth(r) > width {
						row++
						lines = append(lines, []rune{})
					}
