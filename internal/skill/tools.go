...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.roach-code/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.roach-code/skills/ (this workspace only); 'global' writes to ~/.roach-code/skills/ (every project)."
	}
