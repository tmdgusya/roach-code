...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — roach-code setup
	StepSetKey      string // step 2 command label

	// `roach-code init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // roach-code chat step desc
	StepRunDesc    string // roach-code run step desc
	...
	WroteFileFmt          string // "Wrote %s" — used for roach-code.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from roach-code.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: roach-code run [--model NAME] <task>"
...
//
// Priority: override > ROACH_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"ROACH_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
