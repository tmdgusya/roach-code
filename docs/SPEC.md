# Roach Code Engineering Spec

> Roach Code is a coding agent: a thin harness driving multiple models, with **all
...
```
roach-code/
├── go.mod / go.sum          # module roach-code; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── roach-code.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/roach-code/main.go          # entry; blank-imports built-in providers/tools
├── cmd/roach-code-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
...
- Each remote tool is adapted to the `Tool` interface and injected into the run
  ...
  match the MCP tool naming convention and avoid clashes.
...
- `cmd/roach-code-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. Roach Code manages this with
...
- Compaction summarizes the older middle of the session into a single briefing —
  ...
  summarized away.
- The dropped originals are archived to `~/.config/roach-code/archive/<timestamp>.jsonl`
...
- **Custom commands** are Markdown files under `.roach-code/commands/` (project) and
  `~/.config/roach-code/commands/` (user); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — Roach Code
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $ROACH_LANG

[agent]
system_prompt = "You are Roach Code, a coding agent..."  # or system_prompt_file = "..."
max_steps     = 25
temperature   = 0.0
...
`roach-code setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `roach-code.toml` wins (it is the more explicit,
Roach Code-specific source). This lets a server already configured for Claude work in
Roach Code unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o roach-code ./cmd/roach-code`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `roach-code run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `roach-code run`.
