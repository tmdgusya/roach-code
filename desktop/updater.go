...
const (
	...
	manifestFallback    = "https://github.com/tmdgusya/roach-code/releases/latest/download/latest.json"
	defaultDownloadPage = "https://github.com/tmdgusya/roach-code/releases/latest"
...
func applyLinux(targz []byte) error {
	bin, err := extractBinary(targz, "roach-code-desktop")
	if err != nil {
		return err
	}
...
func applyWindows(installer []byte) error {
	f, err := os.CreateTemp("", "roach-code-update-*.exe")
	if err != nil {
		return err
	}
	name := f.Name()
