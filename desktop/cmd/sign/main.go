...
import (
	...
	"roach-code/desktop/internal/update"
)

// platforms are the manifest keys we publish. A built artifact is matched to a key
// by substring (file names embed the key, e.g. Roach Code-darwin-arm64.zip), so the
...
// (roach-code.key) and the public key (roach-code.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "roach-code.key")
	pubPath := filepath.Join(dir, "roach-code.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Roach Code desktop release")
		out := f + ".minisig"
