...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "Roach Code-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	dir := t.TempDir()
	names := []string{
		"Roach Code-darwin-arm64.zip",
		"Roach Code-darwin-amd64.zip",
	...
	t.Setenv("GITHUB_REPOSITORY", "tmdgusya/roach-code")
	...
	wantURL := "https://github.com/tmdgusya/roach-code/releases/download/desktop-v1.2.0/Roach Code-windows-amd64-installer.exe"
	if win.URL != wantURL {
