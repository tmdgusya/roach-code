# Roach Code Desktop (Wails shell)

A native desktop window around the Roach Code Go kernel. The same
...
`desktop/` is its own Go module (`module roach-code/desktop`, `replace roach-code =>
...
`roach-code/` so it can still import the `roach-code/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/Roach Code(.app/.exe)
```
...
- **macOS** — Gatekeeper may report the app "is damaged" or is from an
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Roach Code.app
  ```
...
```sh
minisign -Vm Roach Code-darwin-arm64.zip \
