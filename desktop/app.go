...
import (
	...
	fileenc "roach-code/internal/fileutil/encoding"
...
// of turn into one summary (the "summarize from/up to here" pattern), keeping
...
// folder's roach-code.toml + ROACH-CODE.md, and starts a fresh session — the desktop
...
// custom commands (.roach-code/commands), and MCP prompts — for the composer's "/"
// autocomplete menu.
...
// .roach-code/attachments and returns the relative @-reference path.
...
// as a data URL but not a real path) under .roach-code/attachments and returns the
...
// Memory returns the loaded memory for the panel: the ROACH-CODE.md hierarchy, the
