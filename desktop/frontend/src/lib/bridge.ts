...
export interface AppBindings {
  ...
  // Memory panel: read the loaded ROACH-CODE.md hierarchy + saved auto-memories,
  // quick-add a note to a scope's ROACH-CODE.md (≡ "#<note>"), and overwrite a doc
...
function makeMockApp(): AppBindings {
  let cancelled = false;
  let cwd = "~/projects/roach-code"; // mutable so PickWorkspace is visible in dev
  let workspaces = ["~/projects/roach-code", "~/projects/blade", "~/projects/deepseek-forge", "~/projects/cc-switch-light", "~/projects/SuperRig"];
  let mockEffort = "auto";
  ...
  const capSkills: SkillView[] = [
    ...
    { name: "init", description: "Scaffold a ROACH-CODE.md for this repo", scope: "builtin", runAs: "inline" },
  ];
  let capSkillRoots: SkillRootView[] = [
    { dir: "~/projects/roach-code/.roach-code/skills", scope: "project", priority: 1, status: "missing", configured: false, skills: 0 },
    { dir: "~/my-skills", scope: "custom", priority: 5, status: "ok", configured: true, skills: 1 },
    { dir: "~/.roach-code/skills", scope: "global", priority: 6, status: "ok", configured: false, skills: 2 },
  ];
  ...
  const settings: SettingsView = {
    ...
    agent: { temperature: 0.2, maxSteps: 0, systemPrompt: "You are Roach Code, a coding agent." },
    configPath: "~/projects/roach-code/roach-code.toml",
  ...
  return {
    ...
    async PickWorkspace() {
      ...
      return mockSwitchWorkspace(cwd.endsWith("another-project") ? "~/projects/roach-code" : "~/projects/another-project");
    },
    ...
    async ReadFile(rel: string) {
      const samples: Record<string, string> = {
        "README.md": "# Roach Code\n\nBrowser-dev workspace preview.\n\n- Chat in the center\n- Browse files on the right\n- Keep sessions on the left\n",
        "go.mod": "module roach-code\n\ngo 1.23\n",
    ...
    async SavePastedImage(_dataUrl: string) {
      return ".roach-code/attachments/mock.png";
    },
    async SavePastedFile(name: string, _dataUrl: string) {
      return `.roach-code/attachments/mock-${name}`;
    },
    ...
    async Memory() {
      return {
        available: true,
        storeDir: "~/.config/roach-code/projects/-mock/memory",
        docs: [
          {
            path: "ROACH-CODE.md",
            scope: "project",
            body: "# Roach Code project memory\n\nMock doc shown in the browser dev seam.\n\n## Notes\n\n- prefers concise replies",
          },
          {
            path: "~/.config/roach-code/ROACH-CODE.md",
            scope: "user",
        ...
        scopes: [
          { scope: "user", path: "~/.config/roach-code/ROACH-CODE.md" },
          { scope: "project", path: "ROACH-CODE.md" },
          { scope: "local", path: "ROACH-CODE.local.md" },
        ],
      };
    },
    async Remember(scope: string, note: string) {
      emit({ kind: "notice", level: "info", text: `remembered → ${scope}` });
      return `${scope} ROACH-CODE.md (mock): ${note}`;
    },
    ...
    async CheckUpdate() {
      ...
      return {
        ...
        canSelfUpdate: true,
        downloadUrl: "https://github.com/tmdgusya/roach-code/releases/latest",
    ...
    async OpenDownloadPage() {
      if (typeof window !== "undefined") {
        window.open("https://github.com/tmdgusya/roach-code/releases/latest", "_blank", "noopener");
      }
    },
  };
}
